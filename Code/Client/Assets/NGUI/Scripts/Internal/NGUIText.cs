...
static public class NGUIText
{
    ...
    static public UIDynamicFace DynamicFace = null;
    ...
    static public BMSymbol GetDynamicFace(string text , int index , int textLength)
    {
        return (DynamicFace != null) ? DynamicFace.MatchSymbol(text, index, textLength) : null;
    }
	...
	static public Vector2 CalculatePrintedSize (string text)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
                ...
                if( symbol == null )
                {
                    symbol = GetDynamicFace(text, i, textLength);
                }
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount)
	{
		...
		if (encoding) text = text.Replace("[n]", "\n");
		...
		for (; offset < textLength; ++offset)
		{
            ...
            if (symbol == null)
            {
                symbol = GetDynamicFace(text, offset, textLength);
            }
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			else // No symbol present
			{
                ...
                if( symbol != null )
                {
                    if (DynamicFace != null)
                    {
                        DynamicFace.DrawSymbol(index++, symbol, x, y);
                    }
                    if (Mathf.RoundToInt(x + symbol.advance * fontScale) > rectWidth)
                    {
                        if (x == 0f) return;
	...
	static public void PrintCaretAndSelection (string text, int start, int end, BetterList<Vector3> caret, BetterList<Vector3> highlight)
	{
		...
		for (; index < textLength; ++index)
		{
            ...
            if (symbol == null)
            {
                symbol = GetDynamicFace(text, index, textLength);
            }
