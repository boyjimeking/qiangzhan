...
public class UIPanel : UIRect
{
    ...
    private Vector2 mGameViewSize = Vector2.zero;
	...
	public Vector2 GetViewSize ()
	{
        ...
        if( clip )
        {
            mGameViewSize.x = mClipRange.z;
            mGameViewSize.y = mClipRange.w;
        }else
        {
            mGameViewSize = GetMainGameViewSize();
        }

		//Vector2 size = clip ? new Vector2(mClipRange.z, mClipRange.w) : ;
#else
        ...
        {
            mGameViewSize.x = Screen.width;
            mGameViewSize.y = Screen.height;
        }
#endif
        if (!clip)
		{
			UIRoot rt = root;
#if UNITY_EDITOR
            if (rt != null) mGameViewSize *= rt.GetPixelSizeAdjustment(Mathf.RoundToInt(mGameViewSize.y));
#else
			if (rt != null) mGameViewSize *= rt.GetPixelSizeAdjustment(Screen.height);
#endif
        }
