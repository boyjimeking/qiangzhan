...
public class AIType_1005 : CommonAI
{
    ...
    public AIType_1005(BattleUnit battleUnit)
        : base(battleUnit)
    {
    }
    ...
    {
        return new AIType_1005(battleUnit);
    }
    ...
    private bool CombatChoice()
    {
        ...
        else if (choice == 2 && BaseAI.CheckSkillCd(GetID(), mSkill3) && BaseAI.CheckSkillCost(GetID(), mSkill3))
        {
            if (BeginCommand(2000))
            {
                AddCommand(new AIGoalUseSkillToTargetRangeTime(this, mainTargetId, mSkill2, BaseAI.MoveMode.MOVE_RUN, 30000));
        ...
        else if (choice == 3 && BaseAI.CheckSkillCd(GetID(), mSkill4) && BaseAI.CheckSkillCost(GetID(), mSkill4))
        {
            if (BeginCommand(2000))
            {
                AddCommand(new AIGoalUseSkillToTargetRangeTime(this, mainTargetId, mSkill4, BaseAI.MoveMode.MOVE_RUN, 30000));
            }
            return true;
        }
        else if (choice == 4 && BaseAI.CheckSkillCd(GetID(), mSkill4) && BaseAI.CheckSkillCost(GetID(), mSkill5))
        {
            if (BeginCommand(2000))
            {
                AddCommand(new AIGoalUseSkillToTargetRangeTime(this, mainTargetId, mSkill5, BaseAI.MoveMode.MOVE_RUN, 30000));
            }
            return true;
        }
