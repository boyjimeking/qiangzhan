...
public class AIType_1003 : CommonAI
{
    ...
    public AIType_1003(BattleUnit battleUnit)
        : base(battleUnit)
    {
    }
    ...
    {
        return new AIType_1003(battleUnit);
    }
    ...
    private bool CombatChoice()
    {
        ...
        if (choice == 1 && BaseAI.CheckSkillCd(GetID(), mSkill2) && BaseAI.CheckSkillCost(GetID(), mSkill2))
        {
            if (BeginCommand(2000))
            {
                ...
                Vector3f posm = BaseAI.GetPosition(mainTargetId);
                Vector3f posn = BaseAI.GetPosition(GetID());
                float ranAngle = BaseAI.Randomf(-240, -120);
        ...
        else if (choice == 2 && BaseAI.CheckSkillCd(GetID(), mSkill3) && BaseAI.CheckSkillCost(GetID(), mSkill3))
        {
            if (BeginCommand(2000))
            {
                ...
                AddCommand(new AIGoalUseSkillToTarget(this, mainTargetId, mSkill3));
                for (int i = 0; i < UnityEngine.Random.Range(10, 30); ++i)
                {
                    AddCommand(new AIGoalUseSkillToTargetRangeTime(this, mainTargetId, mWeaponSkillId, BaseAI.MoveMode.MOVE_RUN, 1000));
                }
            }
            return true;
        }
